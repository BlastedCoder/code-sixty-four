...
export default function DraftBoard({ 
  ...
  const all64Picks = Array.from({ length: 64 }).map((_, i) => {
    const pickNum = i + 1;
  ...
  const regions = Array.from(new Set(teams?.map((t: any) => t.region))).filter(Boolean) as string[];
              ...
              {!isDraftComplete && (
                <>
                  <button 
                    ...
                    className={`px-4 py-1.5 text-sm font-bold rounded-lg transition-all shadow-sm border ${
                      isPaused 
          ...
          {all64Picks.map((pick) => {
            ...
            return (
              <div 
                ...
                className={`flex-shrink-0 w-32 md:w-40 rounded-xl p-3 flex flex-col justify-between transition-all snap-center ${
                  isCurrent ? 'bg-emerald-400 border-2 border-emerald-300 transform scale-105 shadow-lg' :
