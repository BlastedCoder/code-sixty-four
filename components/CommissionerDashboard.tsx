...
export default function CommissionerDashboard({ league, members, currentUser }: any) {
  ...
  const handleRandomizeOrder = async () => {
    if (members.length < 2) return toast.error("You need more players to randomize the order!");
  ...
  return (
    <div className="bg-white dark:bg-card border-2 border-slate-900 rounded-2xl p-6 mb-8 shadow-sm">
      <div className="flex items-center space-x-3 mb-6">
        <span className="bg-slate-900 text-white text-xs font-extrabold uppercase tracking-widest px-3 py-1 rounded-full">
          Commissioner
        </span>
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">League Setup Dashboard</h2>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 space-y-4">
          <h3 className="font-bold text-slate-700 dark:text-slate-300">Recruit Players</h3>
          ...
          <form onSubmit={(e) => {
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 bg-slate-50 dark:bg-background border-2 border-slate-400 dark:border-slate-500 rounded-md text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:ring-2 focus:ring-slate-900 focus:border-slate-900 focus:outline-none resize-none shadow-sm"
            />
            <button type="submit" disabled={missingPlayers === 0} className="w-full bg-slate-200 hover:bg-slate-300 text-slate-900 dark:text-white font-extrabold py-2 rounded-md text-sm transition-colors disabled:opacity-50">
              Open Email App
            </button>
          </form>
        </div>
        ...
        <div className="md:col-span-1 space-y-4 border-t md:border-t-0 md:border-l border-slate-200 dark:border-card-border pt-6 md:pt-0 md:pl-8 flex flex-col">
          <div>
            <h3 className="font-bold text-slate-700 dark:text-slate-300">Draft Order</h3>
            <p className="text-xs text-slate-500 dark:text-muted mt-1 mb-3">Set the order manually or let the app randomize it.</p>
          </div>

          <div className="space-y-2 flex-grow overflow-y-auto max-h-48 pr-1 scrollbar-thin scrollbar-thumb-slate-200">
            {localMembers.map((member, index) => (
              <div key={member.user_id} className="flex items-center justify-between bg-slate-50 dark:bg-background p-2 rounded-lg border border-slate-200 dark:border-card-border text-sm">
                <div className="flex items-center space-x-2 truncate pr-2">
                  <span className="font-bold text-slate-400 w-4">{index + 1}.</span>
                  <span className="font-semibold text-slate-700 dark:text-slate-300 truncate">{member.profiles?.display_name || 'Player'}</span>
                </div>
                <div className="flex space-x-1 flex-shrink-0">
                  <button
                    onClick={() => moveMember(index, 'up')}
                    disabled={index === 0 || isProcessing}
                    className="p-1 text-slate-400 hover:text-slate-700 dark:text-slate-300 disabled:opacity-30 transition-colors"
                  >
                    ▲
                  </button>
                  <button
          ...
          <div className="flex space-x-2 pt-2">
            ...
            <button
              ...
              className="flex-1 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 text-slate-800 dark:text-slate-200 font-bold py-2.5 rounded-md text-xs transition-colors disabled:opacity-50"
            >
              🎲 Randomize
            </button>
          </div>
        ...
        {/* Right Column: Launch */}
        <div className="md:col-span-1 space-y-4 border-t md:border-t-0 md:border-l border-slate-200 dark:border-card-border pt-6 md:pt-0 md:pl-8 flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-slate-700 dark:text-slate-300">Launch Draft</h3>
            <p className="text-sm text-slate-500 dark:text-muted">Lock the roster and open the draft board for selections.</p>
          </div>
          <button
