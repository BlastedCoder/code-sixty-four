...
export default function AdminPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-background p-6 md:p-12">
      <div className="max-w-4xl mx-auto bg-white dark:bg-card rounded-2xl shadow-sm border border-slate-200 dark:border-card-border p-6">
        <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white mb-8">Tournament Admin Panel</h1>
        ...
        <div className="space-y-6">
          ...
          {games.filter(g => g.team1_id && g.team2_id && !g.winner_id).length === 0 && (
            <p className="text-slate-500 dark:text-muted">No active games ready to be scored.</p>
          )}
        </div>
      </div>
    </main>
...
function AdminGameCard({ game, team1Name, team2Name, onUpdate }: any) {
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center justify-between p-5 border border-slate-300 dark:border-card-border rounded-xl bg-white dark:bg-card shadow-sm">
      <div className="flex items-center space-x-4 flex-grow mb-4 md:mb-0">
        <span className="font-bold text-slate-600 dark:text-slate-400 w-16">Game {game.id}</span>

        <div className="flex flex-col space-y-3 flex-grow">
          {/* Top Team Input Row */}
          <div className="flex justify-between items-center pr-4 md:pr-8">
            <span className="font-extrabold text-slate-900 dark:text-white text-lg">{team1Name}</span>
            <input
              type="text"
              inputMode="numeric"
              pattern="[0-9]*"
              ...
              placeholder="Score"
              className="w-20 px-3 py-2 border border-slate-400 dark:border-slate-600 rounded-md text-slate-900 dark:text-white bg-white dark:bg-background font-bold focus:outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-emerald-500 text-center"
            />
          </div>

          {/* Bottom Team Input Row */}
          <div className="flex justify-between items-center pr-4 md:pr-8">
            <span className="font-extrabold text-slate-900 dark:text-white text-lg">{team2Name}</span>
            <input
              type="text"
              inputMode="numeric"
              pattern="[0-9]*"
      ...
      <button
        ...
        className="w-full md:w-auto md:ml-4 px-6 py-3 bg-slate-900 dark:bg-emerald-600 hover:bg-slate-800 dark:hover:bg-emerald-500 text-white rounded-md font-bold tracking-wide disabled:opacity-50 transition-colors"
      >
        Submit Final
      </button>
    </div>
